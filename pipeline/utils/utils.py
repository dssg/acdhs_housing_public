...
def get_db_engine(database_creds=None, database=None, pool_size=5, max_overflow=10): # TODO should this be more like the SNAP version of this function
    """
    Sets up SQL engine
    """

    if database_creds is not None: 
...
def get_db_conn(database_creds=None):
    """
    Connects to the sql database
    Returns db connection
    """

    engine = get_db_engine(database_creds)
...
def create_table(db_conn, schema_name, table_name, table_query=None, raise_error=True):
    ...
    Args:
        ...
        raise_error (bool, optional): If there's an error in table creation, raises the exception. Defaults to True. 
    ...
    try:
        ...
        # The pipeline execution is stopped raising a programming error
        if raise_error:
            logging.error(message)
            logging.error(f"Failed to create table {schema_name}.{table_name}")
...
def truncate_table(db_conn, schema_name, table_name):
    """ Truncates an existing table. Can be used to ensure a table is empty for a fresh pipeline run
    Args:
        db_conn (object): Database connection
        schema_name (str): Schema name
        table_name (str): Table name to be inserted into
    ...
    try:
        ...
        logging.warning(f"Failed to truncate table {schema_name}.{table_name}")
...
def add_serial_column(db_conn, schema_name, table_name, serial_column_name):
    
    add_serial_sql = f'''
        alter table {schema_name}.{table_name} add column {serial_column_name} serial;

        commit;
    '''
    db_conn.execute(add_serial_sql)
...
def start_logger(config, experiment_id, pipeline_start_time):
    ...
    logging.basicConfig(
        ...
        level=logging.DEBUG,
        handlers=[
            logging.FileHandler(f'{logging_filepath}/{config.logging.filename}-{sanitized_date}.log'), 
